...
def register_face_gui(return_array=False):
    """Register a face using the camera.

    If return_array is False (default) the function behaves as before: asks for username,
    captures faces, saves a .npy file in USERS_DIR and shows messageboxes.

    If return_array is True the function will return a tuple (mean_face_array, username)
    if capture succeeded, or (None, None) on failure. Caller is responsible for further
    handling (e.g., encrypting and storing in JSON).
    """
...
# NOTA: Esta función no se utiliza actualmente en el flujo de la aplicación
# Se mantiene para posible uso futuro o desarrollo independiente
def train_lbph_model():
    """
    Entrena un modelo LBPH basado en imágenes .jpg guardadas.
    ADVERTENCIA: Esta función no es compatible con el flujo actual 
    que usa archivos .npy para el reconocimiento facial.
    """
...
def load_known_faces():
    """Carga las codificaciones faciales desde archivos .npy"""
    encodings = []
    names = []
