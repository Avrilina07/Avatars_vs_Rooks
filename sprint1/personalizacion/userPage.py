...
class userProfilePage:
    def __init__(self, root):
        ...
        self.userData = self.loadCurrentUser()
        ...
        try:
            ...
            if cf:
                if isinstance(cf, (list, tuple)) and len(cf) >= 3:
                    ...
                    self.bgColor = '#%02x%02x%02x' % (r, g, b)
    ...
    def loadCurrentUser(self):
        ...
        try:
            ...
            if len(users) == 1:
                ...
                # Intentar desencriptar correo para UI
                correo_raw = u.get('correo') or u.get('email') or ''
                correo = correo_raw
                try:
                    if isinstance(correo_raw, str) and Fernet is not None:
                        # localizar clave.key
                ...
                normalized = {
                    ...
                    'apellidos': u.get('apellidos', ''),
                    'username': name,
                    'correo': correo,  # version desencriptada para UI
                    'correo_encrypted': correo_raw,  # mantener version encriptada
        ...
        try:
            ...
            for user in users:
                ...
                if name and name == self.userData.get('username'):
                    ...
                    # Mantener el email encriptado en la BD - no sobrescribir
                    # El email desencriptado solo se usa en la UI (self.userData)
                    pass
    ...
    def createWidgets(self):
        ...
        mainFrame = tk.Frame(self.root, bg=self.bgColor)
        mainFrame.place(relx=0.5, rely=0.5, anchor="center")
        
        # Imagen de perfil
        profileFrame = tk.Frame(mainFrame, bg=self.bgColor)
            ...
            if ruta:
                ...
                if os.path.exists(abs_path):
                    ...
                    try:
                        ...
                        bg_rgb = ImageColor.getrgb(self.bgColor) if hasattr(self, 'bgColor') and self.bgColor else (255, 255, 255)
                    except Exception:
                        bg_rgb = (255, 255, 255)
        ...
        self.profileLabel = tk.Label(profileFrame, image=self.profileImage, bg=self.bgColor, cursor="hand2")
    ...
    def saveChanges(self):
        ...
        try:
            ...
            # Cerrar la ventana actual y volver a pantallaDificultad
            try:
                self.root.destroy()
                
                # Ejecutar el script de pantalla de dificultad
                import runpy
                ...
                if os.path.exists(pd_path):
                    ...
                    print(f"No se encontró pantallaDificultad.py en: {pd_path}")
                    sys.exit(0)
            except Exception as e:
                print(f"Error al volver a pantallaDificultad: {e}")
                sys.exit(0)
    ...
    def changeProfilePhoto(self):
        ...
        if filename:
            try:
                ...
                try:
                    bg_color = self.bgColor if hasattr(self, 'bgColor') and self.bgColor else '#FFFFFF'
                except Exception:
                    bg_color = '#FFFFFF'
